...
#if 0//?callback
    ec = thatCARObject->_classInfo->GetCallbackMethodInfo(Elastos::String(*Utf8String(name)),
            &_callbackMethodInfo);
    if (FAILED(ec))
...
static struct _Value *_ParseValue(Local<Value> value)
{
    ...
    Debug_LOG("OwnPropertyNames count: %d", GetOwnPropertyNames(object).ToLocalChecked()->Length());
...
exit:
    ...
    _value->type = type;
    _value->io = io;
	Debug_LOG("value[%p] data:%p, type:%s, io:%s", *value, *data, type.string(), io_str.string());
...
static void __SetCallerAllocOutputArgumentOfBoolean(IArgumentList *argumentList, size_t index, struct _Value *value)
{
    ...
    unique_ptr<Elastos::Boolean> b(new(nothrow) Elastos::Boolean);
    if (b == nullptr)
        Throw_LOG(Error::NO_MEMORY, 0);
...
{
    Nan::HandleScope scope;
    Elastos::Int32 *__enum;
    ECode ec;

    unique_ptr<Elastos::Int32> enum_(new(nothrow) Elastos::Int32);
    if (enum_ == nullptr)
        Throw_LOG(Error::NO_MEMORY, 0);
        ...
        Nan::FunctionCallback constructor, Local<Value> data)
{
    ...
    Elastos::Int32 nCallbackInterfaces = 0;
#endif
    ...
    for (Elastos::Int32 i = 0; i < nAspects; ++i)
    {
        Nan::HandleScope scope_;
        IClassInfo *aspectInfo;
        Elastos::String aspectName;
    ...
    for (Elastos::Int32 i = 0; i < nAggregatees; ++i)
    {
        Nan::HandleScope scope_;
        IClassInfo *aggregateeInfo;
        Elastos::String aggregateeName;
    ...
    for (Elastos::Int32 i = 0; i < nInterfaces; ++i)
    {
        Nan::HandleScope scope_;
    ...
    for (Elastos::Int32 i = 0; i < nCallbackInterfaces; ++i)
    {
        Nan::HandleScope scope_;
        ICallbackInterfaceInfo *callbackInterfaceInfo;
        Elastos::String callbackInterfaceName;
