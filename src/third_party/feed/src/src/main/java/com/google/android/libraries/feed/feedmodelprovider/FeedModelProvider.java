...
public class FeedModelProvider extends Observable<ModelProviderObserver>
  ...
  public void invalidate() {
    Logger.i(TAG, "Invalidating the current ModelProvider");
    synchronized (lock) {
  ...
  private final Committer<Void, Change> committer =
      new Committer<Void, Change>() {
        ...
        public Void commit(Change change) {
          Logger.i(TAG, "committing ModelProvider mutation");
          ...
          // TODO: When we support synthetic tokens, we won't binding all children.
          ...
          modelChildBinder.bindChildren(
              childrenToBind,
              (bindingCount) -> {
                if (bindingCount != childrenToBind.size()) {
                  Logger.w(TAG, "Not all children added were bound");
                }
          ...
          timeTracker.stop("", "ModelProvider Commit");
          Logger.i(
              TAG,
  ...
  private void handleRemoveOperation(MutationHandler mutationHandler, StreamStructure removeChild) {
    ...
    synchronized (lock) {
      ...
      UpdatableModelChild targetChild =
          Validators.checkNotNull(
              contents.get(childKey), "Child not found in the ModelProvider contents");
      ...
      mainThreadRunner.execute(
          ...
          () -> {
            for (TokenCompletedObserver observer : observers) {
              observer.onChange(tokenCompleted);
            }
          });
    }
  }
