...
public class SessionImplTest {
  ...
  public void testPopulateModelProvider_features() {
    ...
    session.populateModelProvider(TEST_SESSION, streamStructures);
    ...
    assertThatRunnable(
            new ThrowingRunnable() {
              ...
              public void run() throws Throwable {
                session.updateSession(new ArrayList<>(), null);
              }
            })
  ...
  public void testUpdateSession_features() {
    SessionImpl session = getSessionImpl();
    session.populateModelProvider(TEST_SESSION, new ArrayList<>());
    int featureCnt = 3;
  ...
  private SessionImpl getSessionImpl() {
    SessionImpl session =
        new SessionImpl(store, MoreExecutors.newDirectExecutorService(), timingUtils, threadUtils);
